...
pub fn format_arguments(arguments: Option<HashMap<ArgumentType, Vec<Argument>>>, legacy_minecraft_args: Option<String>, mc_args: MinecraftArgs, jvm_args: JvmArgs, main_class: String) -> Vec<String> {
    ...
    if let Some(arguments) = arguments {
        ...
        // Memory
        formatted_args.push(format!("-Xms{}M", jvm_args.min_mem));
        formatted_args.push(format!("-Xmx{}M", jvm_args.max_mem));
...
fn parse_jvm_argument(argument: &str, jvm_args: &JvmArgs) -> String {
    let new = argument
        .replace(
            "${natives_directory}",
        ...
        .replace("${classpath_separator}", if env::consts::OS == "windows" {";"} else {":"})
